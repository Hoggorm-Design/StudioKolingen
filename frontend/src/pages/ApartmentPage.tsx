      ...
      {!isLoading && facility && (
        <>
          <section className="flex flex-col gap-16 px-5 sm:px-10 pt-14">
            ...
            {facility.carouselImages.slice(1).length < 4 && (
              <>
                {/* Mobile Layout */}
                  ...
                  <div className="pt-[138px] pb-[64px] px-5">
                    ...
                    <div className="mb-6">
                      <div className="relative w-full aspect-square overflow-hidden">
                        <img
                          src={facility.carouselImages.slice(1)[0].asset.url}
                          alt={
                            facility.carouselImages.slice(1)[0].altText ||
                            "Additional image"
                          }
                    ...
                    {facility.carouselImages.slice(1).length > 1 && (
                      <div className="flex flex-row justify-center gap-6">
                        {facility.carouselImages
                          .slice(1)
                          .slice(1)
                          .map((image, index) => (
                            <div
                ...
                <section className="w-screen relative left-1/2 -translate-x-1/2 bg-[#1D192C] hidden sm:block">
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 pt-[138px] pb-[64px] lg:py-16 px-5 md:px-36 xl:px-64">
                    {facility.carouselImages.slice(1).map((image, index) => (
            ...
            {facility.carouselImages.slice(1).length >= 4 && (
              <section className="w-screen relative left-1/2 -translate-x-1/2 bg-[#1D192C] py-10">
                <MyCarousel images={facility.carouselImages.slice(1)} />
              </section>
            )}
          </section>
        </>
